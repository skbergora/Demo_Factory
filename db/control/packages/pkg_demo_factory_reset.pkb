create or replace package body pkg_demo_factory_reset as
  ...
  procedure grant_df_healthcheck_privs(p_owner in varchar2) is
  begin
    -- Allow DEMO_FACTORY_ADMIN to reset/seed without requiring TRUNCATE/DROP ANY TABLE.
    execute immediate
      'grant select, insert, update, delete on ' || p_owner || '.DF_HEALTHCHECK to DEMO_FACTORY_ADMIN';
  exception
    when others then
      raise_application_error(-20120, 'Grant on ' || p_owner || '.DF_HEALTHCHECK failed: ' || sqlerrm);
  end;
  ...
  begin
    ...
    grant_df_healthcheck_privs(p_owner);
  end;
  ...
  begin
    ...
    pkg_demo_factory_runs.step_started(p_run_id, 'RESET:ENSURE_BASELINE', 'Ensure baseline objects exist and grants applied');
    ensure_baseline(l_schema);
    pkg_demo_factory_runs.step_succeeded(p_run_id, 'RESET:ENSURE_BASELINE', 'OK');

        pkg_demo_factory_runs.step_started(p_run_id, 'RESET:CLEAR_OBJECTS', 'Truncate/delete allowlisted tenant data');
    begin
